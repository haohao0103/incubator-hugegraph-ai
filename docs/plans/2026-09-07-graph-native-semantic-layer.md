...
**评测暴露的真问题：precision@5 只有 0.293——后来的排查证明这个"问题"一半是假的。**

**P@5 的结构性上限**：gold 表均值仅 1.56 张/例，而 P@5 除以固定 5 个槽位 → **完美检索的上限 = 1.56/5 = 0.311**。实测 0.293 是上限的 **94%**。换句话说，P@5 在这个数据集上几乎没有上升空间，把它当"精度问题"追是误导。据此新增了不受 gold 规模封顶的指标 **excess_ratio**（检收表数/gold 表数）：默认配置 **8.81x**（每张相关表带 8.8 张噪声），`top5/max6` 为 4.14x——这才是真正会动的精度度量。

同时修正了 `precision_at_k` 的实现：原先除以 `min(k, len(retrieved))`，检索单表时会得 1.0 而非标准的 1/5；已改为固定除以 k（标准 P@k），并加测试锁定该语义。

**两轮调参的诚实结论（均无效，但原因值得记录）**：

1. **`min_score_ratio`（扩展表分数下限）**：0.2–0.5 全部产生与 0.0 完全相同的结果。原因是 RRF 是**排名**融合，分数被压缩在窄带里，相对阈值无从判别。种子精度 0.241 vs 扩展精度 0.011 证实噪声主要来自弱种子而非扩展，但排名分数区分不了强弱种子。
2. **`min_seed_ratio`（种子分数下限）**：同样无效（0.3 ≡ 0.0），同一根因。

两个旋钮保留在 API 里（docstring 已如实注明实测无效及原因）：向量召回接入后分数带动态范围会拉开，届时它们才有判别力。**真正有效的杠杆只有两个**：`top_k`/`max_tables`（控制数量），以及未来接入 BM25/向量**分数幅度**做融合（控制质量）。

**配置扫描（同一数据集）**：

| 配置 | P@5 | R@5 | allGold | excess | tokens | saving |
|---|---|---|---|---|---|---|
| 默认 top8/hop2/max12 | 0.293 | **0.963** | **1.000** | 8.81x | 841 | 61.7% |
| hop0/max12 | 0.416 | 0.822 | 0.800 | — | 265 | 87.9% |
| **top5/hop2/max6** | 0.289 | 0.941 | 0.956 | **4.14x** | **418** | **80.9%** |
| hop1/max4 | 0.424 | 0.793 | 0.733 | — | 224 | 89.8% |

**当前保留默认（top8/hop2/max12）的理由**：召回优先。表缺失直接导致查询无法作答且无补救；多余表只多花 token，且有预算器兜底可降级。"excess 8.8x"听着刺眼，但其中大部分是 hop2 扩展来的可 join 邻表——预算器在 token 紧时会先把它们降级成 `name_only`/`keys_only`，而不是丢弃。

**若部署场景 token 敏感**：`top5/hop2/max6` 少 1.9pp 召回、excess 减半、token 再省 31%（累计 80.9%）。但结论只在 ACME 单数据集验证过，改默认前需第二个数据集复核。
