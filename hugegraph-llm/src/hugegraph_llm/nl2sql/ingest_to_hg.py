...
"""Persistent write path: SchemaMetadata JSON -> HugeGraph warehouse-metadata KG.
...
After the writes, :func:`ingest` runs a **deterministic Datalog validation
stage** (:func:`validate_metadata_rules`): lineage transitive closure, caliber
propagation along lineage, caliber-conflict and term-binding-integrity checks.
The inputs are the metadata facts themselves (no LLM, no probabilistic
extractions), so every derived fact is auditable. The report is logged, never
blocks ingestion.

The unified entry point is :class:`~hugegraph_llm.nl2sql.ingest.Nl2SqlIngester`
(``nl2sql/ingest.py``); this module is the low-level writer it calls.
"""
...
LOG_PATH = os.environ.get(
    "HUGEGRAPH_NL2SQL_INGEST_LOG",
    "_out/hg_ingest/logs/ingest.log",
)
...
def ingest(meta: dict, url: str, graph: str, clear: bool = False,
    ...
    for corr in corrections:
        ...
        if not cid:
            continue
        # diff mode: a CorrectionDecision vertex that already exists already
        # has its correctionAppliesTo* edges from a previous run — re-POSTing
        # them is wasted work and breaks idempotency accounting.
        if diff and not _new_correction(cid):
            continue
...
__all__ = [
    "DATALOG_RULES", "bare_table", "ingest", "log",
