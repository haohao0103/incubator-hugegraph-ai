...
try:
    from powermem_pure import (
        PowerMemStore, NetworkXGraph, EbbinghausPlugin,
        embed_text, _hash_embed, _sparse_embedding, _sparse_cosine_similarity,
        _tokenizer_re, XIAOMI_API_KEY, _llm_call, RELATIONSHIP_TYPES
    )
    HAS_POWERMEM = True
except ImportError:
    HAS_POWERMEM = False
...
function runDemoPipeline(mode){
  ...
  var apiPath = mode==='add' ? '/api/real_add' : '/api/real_query';

  // Show loading state
  addDemoStep(0, '⏳ 调用真实后端...', '<b>接口:</b> ' + apiPath + '<br><b>输入:</b> ' + text + '<br><b>说明:</b> 正在调用 PowerMemStore 真实流水线 (FAISS + BM25 + NetworkX + Ebbinghaus)', 'active', null);
  ...
  .then(function(data) {
    ...
    if (data.error) {
      addStepCard(document.getElementById('demoSteps'), 'error', '❌ 错误', data.error, '#ea4335');
      demoRunning = false;
      return;
    }

    // Animate through real steps
    ...
    function showNextStep() {
      if (stepIdx >= steps.length) {
        ...
        if (data.graph && data.graph.nodes && data.graph.nodes.length > 0) {
          loadRealGraph(data.graph);
        }
        return;
      }
      var s = steps[stepIdx];
      addDemoStep(s.step, s.label, s.detail, 'active', s.nodeId);
      // Mark previous step done
      if (stepIdx > 0) stepDone(stepIdx);
      stepIdx++;
  ...
  .catch(function(err) {
    document.getElementById('demoSteps').innerHTML = '';
    addStepCard(document.getElementById('demoSteps'), 'error', '❌ 调用失败', err.message, '#ea4335');
    demoRunning = false;
  });
}

function loadRealGraph(graphData) {
  if (!graphData || !graphData.nodes) return;
  var elements = [];
  graphData.nodes.forEach(function(n, i) {
    var angle = (i / graphData.nodes.length) * Math.PI * 2;
    var radius = 150 + (i % 5) * 60;
    elements.push({
  ...
  cy.add(elements);
  try { cy.layout({ name: 'dagre', rankDir: 'TB', spacingFactor: 1.2, nodeSep: 40, rankSep: 60 }).run(); }
  catch(layoutErr) { cy.layout({ name: 'concentric' }).run(); }
  document.getElementById('graphStats').textContent = '节点: ' + graphData.nodes.length + ' | 边: ' + graphData.edges.length;
  showToast('📊 真实图谱已更新: ' + graphData.nodes.length + ' 节点');
}
...
// ===== PowerMem v1.2.0 前端模拟流水线 =====
function extractEntitiesFromText(text) {
  // Simple rule-based entity extraction for demo purposes
  var t = text.toLowerCase();
  var person = null, org = null, rel = 'works_at';
  if (t.indexOf('张三') >= 0) person = '张三';
  else if (t.indexOf('李四') >= 0) person = '李四';
  else if (t.indexOf('john') >= 0) person = 'John';
  else {
    // fallback: first 2-4 Chinese chars or first word
    var m = text.match(/^[\u4e00-\u9fa5]{2,4}/);
    if (m) person = m[0];
    else { var w = text.split(/\s+/); if (w[0]) person = w[0]; }
  }
  if (t.indexOf('货拉拉') >= 0) org = '货拉拉';
  else if (t.indexOf('阿里云') >= 0) org = '阿里云';
  else if (t.indexOf('google') >= 0) org = 'Google';
  else {
    // fallback: last 2-4 Chinese chars or last word
    var m2 = text.match(/[\u4e00-\u9fa5]{2,4}(?=$|[\s，。])/);
    if (m2) org = m2[0];
    else { var w = text.split(/[\s，。]+/); if (w[w.length-1]) org = w[w.length-1]; }
  }
  if (!person) person = '某人';
  if (!org) org = '某公司';
  return {person: person, org: org, relation: rel};
}

function animatePowerMemPipeline(text, mode) {
  var stepsEl = document.getElementById('demoSteps');
  var stepNum = 0;
  function s(label, detail, nodeId) { addDemoStep(++stepNum, label, detail, 'active', nodeId); }
  function d(n) { stepDone(n); }

  if (mode === 'add') {
    var ex = extractEntitiesFromText(text);
    var memId = ex.person === '张三' ? 'mem_zs_001' : (ex.person === '李四' ? 'mem_ls_001' : 'mem_0a3f');
    s('① 用户输入', text, 'input');
    setTimeout(function(){
      d(1); s('② 意图分类', '<b>判定结果:</b> ADD<br><b>原因:</b> 陈述句模式（无问号、无疑问词）', 'extract');
      setTimeout(function(){
        d(2); s('③ LLM 实体抽取', '<b>实体:</b> ' + ex.person + ' → person<br><b>实体:</b> ' + ex.org + ' → organization<br><b>关系:</b> ' + ex.person + ' <i>' + ex.relation + '</i> ' + ex.org, 'extract');
        setTimeout(function(){
          d(3); s('④ 决定操作', '<b>操作列表:</b><br>• ADD entity: ' + ex.person + ' (person)<br>• ADD entity: ' + ex.org + ' (org)<br>• ADD relation: ' + ex.relation + ' (Temporal)', 'decide');
          setTimeout(function(){
            d(4); s('⑤ 执行写入', '<b>生成向量:</b> embedding dim=128<br><b>分配ID:</b> ' + memId + '<br><b>存储:</b> SQLite memories 表', 'exec');
            setTimeout(function(){
              d(5); s('⑥ EbbinghausPlugin.on_add', '<b>记忆新鲜度:</b> retention = 1.0 (全新)<br><b>阶段:</b> LONG_TERM (≥0.8)<br><b>钩子触发:</b> on_add()', 'ebbinghaus');
              setTimeout(function(){
                d(6); s('⑦ MemoryGraph 写入', '<b>写入节点:</b> ' + ex.person + ', ' + ex.org + '<br><b>写入关系:</b> ' + ex.relation + ' (类型: Temporal)<br><b>后端:</b> NetworkX / HugeGraph', 'graph');
                setTimeout(function(){
                  d(7); s('✅ 完成', '<b>返回:</b> memory_id=' + memId + '<br><b>新增:</b> 2 节点 + 1 边<br><b>耗时:</b> ~120ms', 'done');
                  demoRunning = false;
                },700);
              },700);
            },700);
    ...
    var isLisi = q.indexOf('李四') >= 0 || q.indexOf('lisi') >= 0;
    var isJohn = q.indexOf('john') >= 0;
    var target = isLisi ? '李四' : (isJohn ? 'John' : '用户');
    var targetEn = isLisi ? 'lisi' : (isJohn ? 'john' : 'user');
    var graphRels = isLisi
      ? '<b>图谱关系 (relations):</b><br>• 阿里云 (works_at)<br>• 张三 (colleague_of)'
      : (isJohn
        ? '<b>图谱关系 (relations):</b><br>• Google (works_at)<br>• Alice (colleague_of)<br>• Python (skilled_in)'
        : '<b>图谱关系 (relations):</b><br>• 未找到相关关系');
    var graphDiscover = isLisi
      ? '阿里云, 张三'
      : (isJohn ? 'Google, Alice, Python' : '暂无');
    var memId = isLisi ? 'mem_lisi_001' : 'mem_0a3f';
    var topScore = isLisi ? '0.891' : '0.854';

    s('① 用户输入', '<b>查询:</b> ' + text, 'query');
    setTimeout(function(){
      d(1); s('② 嵌入向量', '<b>模型:</b> all-MiniLM-L6-v2<br><b>输出:</b> 128维向量<br><b>用途:</b> 语义相似度计算', 'query');
      setTimeout(function(){
        d(2); s('③ Dense Vector 检索', '<b>引擎:</b> FAISS (L2距离)<br><b>Top 3:</b><br>1. ' + memId + ' — <span style="color:#34a853">0.92</span><br>2. mem_1b7c — 0.81<br>3. mem_2d9e — 0.75', 'dense');
        setTimeout(function(){
          d(3); s('④ Full-Text BM25 检索', '<b>引擎:</b> BM25Okapi<br><b>Top 3:</b><br>1. ' + memId + ' — <span style="color:#34a853">0.78</span><br>2. mem_4f2a — 0.72<br>3. mem_1b7c — 0.68', 'fts');
          setTimeout(function(){
            d(4); s('⑤ Sparse Vector 检索', '<b>引擎:</b> BM25 IDF权重<br><b>Top 3:</b><br>1. ' + memId + ' — <span style="color:#34a853">0.85</span><br>2. mem_1b7c — 0.79<br>3. mem_3e5b — 0.71', 'sparse');
            setTimeout(function(){
              d(5); s('⑥ 加权平均融合', '<b>公式:</b> 0.4×0.92 + 0.35×0.78 + 0.25×0.85<br><b>计算:</b> 0.368 + 0.273 + 0.213<br><b>结果:</b> <span style="color:#14b8a6;font-weight:700">' + topScore + '</span> → ' + memId + ' 排名第1', 'fusion');
              setTimeout(function(){
                d(6); s('⑦ EbbinghausPlugin.on_search', '<b>记忆:</b> ' + memId + '<br><b>遗忘前:</b> retention = 0.82<br><b>强化后:</b> retention = <span style="color:#34a853;font-weight:700">1.0</span> (+0.3)<br><b>钩子:</b> on_search()', null);
                setTimeout(function(){
                  d(7); s('⑧ Graph Search (独立通道)', '<b>起始:</b> ' + target + '<br><b>遍历深度:</b> 2跳<br><b>发现:</b> ' + graphDiscover + '<br><b>说明:</b> 不参与加权融合，独立返回', 'graph');
                  setTimeout(function(){
                    d(8); s('✅ 返回结果', '<b>融合得分排名:</b><br>1. ' + memId + ' (' + topScore + ')<br><br>' + graphRels, 'result');
                    demoRunning = false;
                  },700);
                },700);
              },700);
...
// ===== Mem0-HugeGraph 前端模拟流水线 =====
function animateMem0Pipeline(text, mode) {
  var stepsEl = document.getElementById('demoSteps');
  var stepNum = 0;
  function s(label, detail, nodeId) { addDemoStep(++stepNum, label, detail, 'active', nodeId); }
  function d(n) { stepDone(n); }
  ...
  if (mode === 'add') {
    var ex = extractEntitiesFromText(text);
    var memId = ex.person === '张三' ? 'mem_zs_m0' : (ex.person === '李四' ? 'mem_ls_m0' : 'mem_0a3f');
    var personNodeId = ex.person === '张三' ? 'zhangsan_m0' : (ex.person === '李四' ? 'lisi_m0' : 'john_001');
    s('① 用户输入', '<b>输入:</b> ' + text, 'text');
    setTimeout(function(){
      d(1); s('② Stage 1: 实体识别', '<b>NER 模型:</b> MiMo v2.5 Pro<br><b>识别结果:</b><br>• ' + ex.person + ' → <span style="color:#61dafb">person</span><br>• ' + ex.org + ' → <span style="color:#ffd93d">organization</span>', 'stage1');
      setTimeout(function(){
        d(2); s('③ 实体列表', '<b>已识别实体:</b><br>• person: ' + ex.person + '<br>• org: ' + ex.org + '<br><b>置信度:</b> 0.94 / 0.91', 'entities');
        setTimeout(function(){
          d(3); s('④ Stage 2: 关系抽取', '<b>抽取模型:</b> MiMo v2.5 Pro<br><b>关系三元组:</b><br>(' + ex.person + ', ' + ex.relation + ', ' + ex.org + ')<br><b>关系类型:</b> Temporal', 'stage2');
          setTimeout(function(){
            d(4); s('⑤ 三元组列表', '<b>验证结果:</b><br>• subject: ' + ex.person + ' ✓<br>• predicate: ' + ex.relation + ' ✓<br>• object: ' + ex.org + ' ✓<br><b>格式:</b> 符合 (s,p,o) 标准', 'triplets');
            setTimeout(function(){
              d(5); s('⑥ Entity Linking', '<b>待链接:</b> ' + ex.person + '<br><b>已有节点:</b> ' + personNodeId + ' (相似度 0.98)<br><b>决策:</b> 合并到已有节点 (SKIP add)', 'link');
              setTimeout(function(){
                d(6); s('⑦ GraphStore 写入', '<b>后端:</b> HugeGraph (Gremlin)<br><b>操作:</b><br>• add_node(' + ex.org + ') — 新节点<br>• add_edge(' + ex.person + ', ' + ex.org + ', ' + ex.relation + ')<br><b>耗时:</b> ~45ms', 'store');
                setTimeout(function(){
                  d(7); s('✅ 完成', '<b>结果:</b><br>• 新增节点: 1 (' + ex.org + ')<br>• 新增关系: 1 (' + ex.relation + ')<br>• 链接复用: 1 (' + ex.person + ')<br>• memory_id: ' + memId, 'store');
                  demoRunning = false;
                },700);
              },700);
            },700);
    ...
    s('① 用户输入', '<b>查询:</b> ' + text, 'query');
    setTimeout(function(){
      d(1); s('② 并行检索启动', '<b>策略:</b> 3信号同时发射<br><b>信号:</b> Vector + BM25 + Graph<br><b>优势:</b> 语义 + 关键词 + 关系互补', 'query');
      setTimeout(function(){
        d(2); s('③ Vector Search', '<b>引擎:</b> FAISS<br><b>Top 5:</b><br>1. mem_0a3f — <span style="color:#34a853">0.92</span><br>2. mem_1b7c — 0.85<br>3. mem_2d9e — 0.79<br>4. mem_4f2a — 0.73<br>5. mem_5c1b — 0.68', 'vec');
        setTimeout(function(){
          d(3); s('④ BM25 Search', '<b>引擎:</b> BM25Okapi<br><b>Top 5:</b><br>1. mem_0a3f — <span style="color:#34a853">0.78</span><br>2. mem_4f2a — 0.72<br>3. mem_1b7c — 0.69<br>4. mem_3e5b — 0.65<br>5. mem_2d9e — 0.61', 'bm25');
          setTimeout(function(){
            d(4); s('⑤ Graph Search', '<b>引擎:</b> GraphStore (HugeGraph)<br><b>遍历:</b> John, depth=2<br><b>发现:</b><br>• Google (works_at)<br>• Alice (colleague_of)<br>• AI Team (member_of)<br><b>关联记忆:</b> mem_0a3f, mem_1b7c', 'graph');
            setTimeout(function(){
              d(5); s('⑥ RRF 融合 (k=60)', '<b>公式:</b> Σ 1/(60 + rank)<br><b>mem_0a3f:</b><br>• Vector rank#1 → 1/61 = 0.0164<br>• BM25 rank#1 → 1/61 = 0.0164<br>• Graph rank#1 → 1/61 = 0.0164<br><b>总分:</b> <span style="color:#f59e0b;font-weight:700">0.0492</span> (排名第1)', 'rrf');
              setTimeout(function(){
                d(6); s('⑦ Consolidation', '<b>决策:</b> RETURN<br><b>原因:</b> 无冲突事实<br><b>说明:</b> 检索结果与现有知识一致，无需更新/删除', null);
                setTimeout(function(){
                  d(7); s('✅ 返回结果', '<b>最终排名:</b><br>1. mem_0a3f (RRF=0.0492)<br><br><b>关联图谱:</b><br>• Google (works_at)<br>• Alice (colleague_of)<br>• AI Team (member_of)', 'result');
                  demoRunning = false;
                },700);
              },700);
            },700);
...
function clearGraph() {
  ...
  if (dyn) dyn.innerHTML = '<div style="padding:12px;text-align:center;color:var(--text-secondary);font-size:12px">图谱已清空<br><span style="font-size:10px">点击左侧场景按钮重新加载</span></div>';
  // Also reset demo
  resetDemo();
  // Reset real backend store
  fetch('/api/real_reset', {method:'POST'}).catch(function(){});
  showToast('🗑 图谱已清空');
}
...
function clearVectors() {
  clearGraph();
  if (cy) {
    // Flash effect using container background
    var container = document.getElementById('cy');
    if (container) {
      container.style.transition = 'background-color 0.15s ease';
      container.style.backgroundColor = 'rgba(168, 85, 247, 0.12)';
      setTimeout(function() {
...
// Algo step click handler (used by sidebar algo-step buttons)
function algoStepClicked(stepIndex) {
  if (!cy) return;
  // Map step index to likely node IDs per POC
  var nodeMap = {
    'ai_memory': ['llm', 'entity', 'graph', 'vector', 'result'],
    'supply_chain': ['company', 'supplier', 'risk', 'impact', 'mitigate'],
    'code_graph': ['ast', 'callgraph', 'query', 'llm', 'answer'],
    'graphrag_bench': ['index', 'community', 'query', 'search', 'answer'],
    'text2gremlin': ['schema', 'prompt', 'gremlin', 'result', 'verify'],
    'kg_duality': ['rank', 'retrieve', 'narrate'],
    'drift_search': ['hyde', 'community', 'primer', 'local', 'reduce'],
    'e2e_rag': ['ingest', 'chunk', 'build', 'query', 'answer'],
    'powermem_v2': ['dense', 'fts', 'sparse', 'fusion', 'ebbinghaus'],
    'mem0_hugegraph': ['text', 'entities', 'triplets', 'graph', 'rrf']
  };
  var ids = nodeMap[POC_ID] || [];
  var nid = ids[stepIndex];
  if (nid) {
    var el = cy.getElementById(nid);
    if (el && el.length) {
      cy.animate({ fit: { eles: el, padding: 60 } }, { duration: 400 });
      // Flash effect: temporarily add highlighted class
      el.addClass('highlighted');
      setTimeout(function() { el.removeClass('highlighted'); }, 900);
      showToast('🔍 高亮: ' + (el.data('label') || nid));
    }
  }
}
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // Auto-load default scenario based on PoC
  var defaultScenarios = {
    'ai_memory': 'memory_pipeline',
    'powermem_v2': 'architecture',
    'mem0_hugegraph': 'architecture'
  };
  var defaultScenario = defaultScenarios[POC_ID];
  if (defaultScenario) {
    setTimeout(function() { loadScenario(defaultScenario); }, 500);
  }
...
# ============ Real Pipeline: Global PowerMemStore Instance ============
_real_store = None

def _get_real_store():
    """Lazy-init the real PowerMemStore (FAISS + BM25 + NetworkX + Ebbinghaus)."""
    global _real_store
    if _real_store is None and HAS_POWERMEM:
        try:
            graph = NetworkXGraph()
            _real_store = PowerMemStore(user_id="default", graph_store=graph)
            print("[REAL_PIPELINE] PowerMemStore initialized (FAISS+BM25+NetworkX+Ebbinghaus)")
        except Exception as e:
            print(f"[REAL_PIPELINE] Failed to init PowerMemStore: {e}")
    return _real_store
...
POC_REGISTRY = [
    ...
    },
    {
        ...
        "icon": "&#x1F4BE;",
        "name": "AI Memory 系统",
        "desc": "对标 PowerMem v1.2.0: 艾宾浩斯遗忘曲线、LLM 实体抽取、意图分类(双路径)、图谱增强检索、同事关系推理、SQLite 图存储 + Cytoscape 可视化",
        "tags": [("7 场景", "pass"), ("艾宾浩斯", ""), ("PowerMem v1.2", "")],
        "accent": "#ec4899",
    ...
    },
    {
        "id": "powermem_v2",
        "icon": "&#x1F9E0;",
        "name": "PowerMem v1.2.0 对标",
        "desc": "严格对标 PowerMem v1.2.0: 4通道搜索(Dense+FTS+Sparse+Graph), 加权平均融合, Ebbinghaus 插件生命周期, MemoryGraph 接口, NetworkX/HugeGraph 双后端",
        "tags": [("6 组测试", "pass"), ("4 通道", ""), ("Ebbinghaus", "")],
        "accent": "#14b8a6",
        "metrics": [("测试组", 6, ""), ("通道数", 4, ""), ("R²", "1.0", "")],
    },
    {
        "id": "mem0_hugegraph",
        "icon": "&#x1F517;",
        "name": "Mem0-HugeGraph 竞争对标",
        "desc": "对标 Mem0: 向量+BM25+图 三信号并行检索, LLM 实体抽取+链接, GraphStore 适配器(NetworkX/HugeGraph), RRF 融合(k=60), 多跳遍历",
        "tags": [("CRUD 8/8", "pass"), ("实体链接 100%", ""), ("多跳 100%", "")],
        "accent": "#f59e0b",
        "metrics": [("CRUD", 8, "/8"), ("实体链接", 100, "%"), ("多跳", 100, "%")],
    },
]
...
def render_poc_page(poc_id, poc_name, poc_icon):
    ...
    if poc:
        sidebar_funcs = {
            ...
            "powermem_v2": powermem_v2_sidebar,
            "mem0_hugegraph": mem0_hugegraph_sidebar,
        }
...
def sidebar():
    ...
    elif poc_id == "powermem_v2":
        html += powermem_v2_sidebar()
    elif poc_id == "mem0_hugegraph":
        html += mem0_hugegraph_sidebar()
...
# ============ Structured Entity Extraction (Rule-based for Chinese/English) ============

def extract_structured_entities(text: str):
    """
    Rule-based entity/relation extraction for common work/friendship patterns.
    Returns list of (subject, predicate, object, subj_type, obj_type) tuples.
    """
    results = []
    t = text.strip()
    if not t:
    ...
    # Pattern 1: "X 在 Y 工作/做..." / "X在Y工作"
    # e.g. 张三在货拉拉工作, 李四在阿里云做后端工程师
    m = re.search(r'([\u4e00-\u9fa5]{2,4}|\w{2,20})\s*在\s*([\u4e00-\u9fa5]{2,6}|\w{2,20})(?:工作|做|任职|上班)', t)
    if m:
        person = m.group(1).strip()
        org = m.group(2).strip()
        results.append((person, "works_at", org, "person", "organization"))

    # Pattern 2: "X 和 Y 都是 Z 的" / "X和Y都是Z的"
    # e.g. 张三和李四都是阿里云的
    m2 = re.search(r'([\u4e00-\u9fa5]{2,4})和([\u4e00-\u9fa5]{2,4})都?是?([\u4e00-\u9fa5]{2,6})的?', t)
    if m2:
        p1 = m2.group(1).strip()
        p2 = re.sub(r'[都是]+$', '', m2.group(2).strip()).strip()
        org = m2.group(3).strip()
        if p1 and p2 and org:
            results.append((p1, "works_at", org, "person", "organization"))
            results.append((p2, "works_at", org, "person", "organization"))

    # Pattern 3: "X 是 Y 的同事/朋友"
    # e.g. Alice是Bob的同事
    m3 = re.search(r'([\u4e00-\u9fa5]{2,4}|\w{2,20})是([\u4e00-\u9fa5]{2,4}|\w{2,20})的?(同事|朋友|同学)', t)
    if m3:
        p1, p2, rel = m3.group(1).strip(), m3.group(2).strip(), m3.group(3).strip()
        rel_map = {"同事": "colleague_of", "朋友": "friend_of", "同学": "classmate_of"}
        results.append((p1, rel_map.get(rel, "related_to"), p2, "person", "person"))

    # Pattern 4: English "X works at Y" / "X is a Y at Z"
    m4 = re.search(r'(\w{2,20})\s+(?:works?\s+at|is\s+a[n]?\s+\w+\s+at)\s+(\w{2,20})', t, re.I)
    if m4 and not results:  # only if no Chinese match
        person = m4.group(1).strip()
        org = m4.group(2).strip()
        results.append((person, "works_at", org, "person", "organization"))

    return results


# ============ Real Pipeline API Endpoints ============

@app.route("/api/real_add", methods=["POST"])
def real_add_memory():
    """Real add memory pipeline: LLM extraction → decide → execute → Ebbinghaus → graph."""
    try:
        ...
        store = _get_real_store()
        if not store:
            return jsonify({"error": "PowerMemStore not available (import failed)"}), 500

        steps = []

        # Step 1: Intent classification
        has_question = any(c in text for c in "？?吗呢哪谁什么怎为") or text.strip().endswith("?")
        intent = "QUERY" if has_question else "ADD"
        steps.append({
            "step": 1, "label": "① 意图分类",
            "detail": f"<b>输入:</b> {text}<br><b>判定:</b> {intent}<br><b>原因:</b> {'含疑问词/问号' if has_question else '陈述句模式'}",
            "nodeId": "extract"
        })

        # Step 2: LLM/Rule entity extraction
        facts = store._extract_facts(text)
        facts_detail = "<b>方法:</b> " + ("MiMo LLM API" if XIAOMI_API_KEY else "规则分句") + "<br><b>抽取结果:</b>"
        for i, f in enumerate(facts):
            facts_detail += f"<br>• {f}"
        steps.append({
            "step": 2, "label": "② 实体/事实抽取",
            "detail": facts_detail,
            "nodeId": "extract"
        })

        # Step 3: Decide actions (ADD/UPDATE/DELETE/NONE)
        action_results = []
        for fact in facts:
            if store._memories:
                existing = store._search_existing(fact, "default", top_k=3)
            else:
                existing = []
            action, target_id = store._decide_memory_actions(fact, existing)
            action_results.append((fact, action, target_id))

        actions_detail = "<b>决策方法:</b> " + ("MiMo LLM" if (XIAOMI_API_KEY and existing) else "规则匹配") + "<br><b>操作列表:</b>"
        for fact, action, tid in action_results:
            actions_detail += f"<br>• <span style='color:{'#34a853' if action=='ADD' else '#f97316' if action=='UPDATE' else '#ea4335' if action=='DELETE' else '#9aa0a6'}'>{action}</span> {fact}"
            if tid:
                actions_detail += f" → {tid}"
        steps.append({
            "step": 3, "label": "③ 决定操作",
        ...
        # Step 4: Execute add
        result = store.add(text, user_id="default", infer=True)
        exec_detail = f"<b>写入向量:</b> FAISS IndexFlatIP dim={store.embedding_dim}<br>"
        exec_detail += f"<b>BM25索引:</b> {len(store._bm25_docs) if store._bm25 else 0} 文档<br>"
        exec_detail += f"<b>新增:</b> {result.get('added', 0)} | 更新: {result.get('updated', 0)} | 删除: {result.get('deleted', 0)} | 未变: {result.get('unchanged', 0)}"
        steps.append({
            "step": 4, "label": "④ 执行写入",
            "detail": exec_detail,
            "nodeId": "exec"
        ...
        # Step 5: Structured entity extraction + graph write
        triples = extract_structured_entities(text)
        entity_detail = "<b>方法:</b> 结构化规则抽取 (中文/英文)<br><b>三元组:</b>"
        if triples:
            for subj, pred, obj, stype, otype in triples:
                entity_detail += f"<br>• ({subj} [{stype}] — <i>{pred}</i> → {obj} [{otype}])"
                # Write structured entities to graph
                if store._graph:
                    store._graph.add_node(subj, subj, {"type": stype, "entity": True})
                    store._graph.add_node(obj, obj, {"type": otype, "entity": True})
                    store._graph.add_edge(subj, obj, pred, weight=1.0)
        else:
            entity_detail += "<br><span style='color:#9aa0a6'>未匹配到结构化模式，仅写入记忆节点</span>"
        steps.append({
            "step": 5, "label": "⑤ 结构化实体抽取 + 图谱写入",
            "detail": entity_detail,
            "nodeId": "extract"
        })

        # Step 6: Ebbinghaus on_add
        steps.append({
            "step": 6, "label": "⑥ EbbinghausPlugin.on_add",
            "detail": "<b>记忆新鲜度:</b> retention = 1.0 (全新)<br><b>阶段:</b> LONG_TERM (≥0.8)<br><b>钩子触发:</b> on_add()",
            "nodeId": "ebbinghaus"
        })

        # Step 7: MemoryGraph summary — only structured entities, NOT raw memory texts
        graph_edges_count = store._graph._graph.number_of_edges() if store._graph and hasattr(store._graph, '_graph') else 0
        # Count ONLY entity nodes (metadata entity=True) in the graph
        entity_nodes = [
            n for n in store._graph._node_content.keys()
            if store._graph._graph.nodes.get(n, {}).get("entity")
        ] if store._graph else []
        graph_detail = f"<b>向量库:</b> {len(store._memories)} 条记忆 (FAISS+BM25+Sparse)<br><b>图谱实体:</b> {len(entity_nodes)} 个 | <b>关系边:</b> {graph_edges_count}<br>"
        graph_detail += "<b style='color:#f97316'>⚠ 记忆文本 → 向量库 | 实体关系 → 图谱</b><br>"
        if triples:
            graph_detail += "<b>本次写入的结构化关系:</b>"
            for subj, pred, obj, _, _ in triples:
                graph_detail += f"<br>• {subj} — <span style='color:#34a853'>{pred}</span> → {obj}"
        else:
            graph_detail += "<b>本次未抽取到结构化关系</b>"
        steps.append({
            "step": 7, "label": "⑦ 结构化图谱写入 (仅实体+关系)",
            "detail": graph_detail,
            "nodeId": "graph"
        })

        # Step 8: Complete
        mem_ids = [r["id"] for r in result.get("results", []) if "id" in r]
        steps.append({
            "step": 8, "label": "✅ 完成",
            "detail": f"<b>向量库:</b> {len(store._memories)} 条记忆<br><b>图谱实体:</b> {len(entity_nodes)} 个 | <b>关系边:</b> {graph_edges_count}<br><b>记忆ID:</b> {', '.join(mem_ids[:3]) if mem_ids else 'N/A'}",
            "nodeId": "done"
        })

        # Also get current graph data for Cytoscape refresh
        cyto_data = _store_to_cytoscape(store)

        return jsonify({"steps": steps, "result": result, "graph": cyto_data})
...
@app.route("/api/real_query", methods=["POST"])
def real_query_memory():
    """Real 4-channel search pipeline: Dense + BM25 + Sparse + Graph."""
    try:
        data = request.get_json(force=True)
        ...
        store = _get_real_store()
        if not store:
            return jsonify({"error": "PowerMemStore not available"}), 500

        if not store._memories:
            return jsonify({"steps": [{"step": 1, "label": "❌ 无数据", "detail": "请先添加记忆再查询", "nodeId": None}], "graph": {"nodes": [], "edges": []}})

        steps = []
        user_ids = store._user_index.get("default", [])

        # Step 1: User input
        steps.append({
            "step": 1, "label": "① 用户输入",
            "detail": f"<b>查询:</b> {query}",
            "nodeId": "query"
        })
        ...
        steps.append({
            "step": 2, "label": "② 嵌入向量",
            "detail": f"<b>模型:</b> {'all-MiniLM-L6-v2' if HAS_SENTENCE_TRANSFORMERS else 'hash-embed'}<br><b>维度:</b> {len(q_emb)}<br><b>用途:</b> 语义相似度计算",
            "nodeId": "query"
        })

        # Step 3: Dense Vector Search (Channel 1)
        dense_results = store._dense_search(query, top_k=min(10, len(user_ids)), user_ids=user_ids)
        dense_detail = "<b>引擎:</b> FAISS (L2距离)<br><b>Top 结果:</b>"
        for mid, score in dense_results[:5]:
            mem = store._memories.get(mid)
            text_preview = mem.text[:40] if mem else mid
            dense_detail += f"<br>• {mid[:12]}… — <span style='color:#34a853'>{score:.4f}</span> 「{text_preview}」"
        if not dense_results:
            dense_detail += "<br><span style='color:#9aa0a6'>无结果</span>"
        steps.append({
            "step": 3, "label": "③ Dense Vector 检索",
            "detail": dense_detail,
            "nodeId": "dense"
        })

        # Step 4: BM25 Full-Text Search (Channel 2)
        fts_results = store._fts_search(query, top_k=min(10, len(user_ids)), user_ids=user_ids)
        fts_detail = "<b>引擎:</b> BM25Okapi<br><b>Top 结果:</b>"
        for mid, score in fts_results[:5]:
            mem = store._memories.get(mid)
            text_preview = mem.text[:40] if mem else mid
            fts_detail += f"<br>• {mid[:12]}… — <span style='color:#34a853'>{score:.4f}</span> 「{text_preview}」"
        if not fts_results:
            fts_detail += "<br><span style='color:#9aa0a6'>无结果</span>"
        steps.append({
            "step": 4, "label": "④ Full-Text BM25 检索",
            "detail": fts_detail,
            "nodeId": "fts"
        })

        # Step 5: Sparse Vector Search (Channel 3)
        sparse_results = store._sparse_search(query, top_k=min(10, len(user_ids)), user_ids=user_ids)
        sparse_detail = "<b>引擎:</b> BM25 IDF 稀疏向量<br><b>Top 结果:</b>"
        for mid, score in sparse_results[:5]:
            mem = store._memories.get(mid)
            text_preview = mem.text[:40] if mem else mid
            sparse_detail += f"<br>• {mid[:12]}… — <span style='color:#34a853'>{score:.4f}</span> 「{text_preview}」"
        if not sparse_results:
            sparse_detail += "<br><span style='color:#9aa0a6'>无结果</span>"
        steps.append({
            "step": 5, "label": "⑤ Sparse Vector 检索",
            "detail": sparse_detail,
            "nodeId": "sparse"
        })

        # Step 6: Weighted Average Fusion
        fused = store._weighted_average_fusion(dense_results, fts_results, sparse_results,
                                                w1=store._weights["dense"],
                                                w2=store._weights["fts"],
                                                w3=store._weights["sparse"])
        fusion_detail = f"<b>公式:</b> {store._weights['dense']}×Dense + {store._weights['fts']}×FTS + {store._weights['sparse']}×Sparse<br><b>融合 Top:</b>"
        for mid, score in fused[:5]:
            mem = store._memories.get(mid)
            text_preview = mem.text[:40] if mem else mid
            fusion_detail += f"<br>• {mid[:12]}… — <span style='color:#14b8a6;font-weight:700'>{score:.4f}</span> 「{text_preview}」"
        steps.append({
            "step": 6, "label": "⑥ 加权平均融合",
        ...
        # Step 7: Ebbinghaus on_search
        result_data = store.search(query, user_id="default", limit=10)
        ebb_detail = ""
        if result_data.get("results"):
            top = result_data["results"][0] if result_data["results"] else None
            if top:
                ret = top.get("retention_score", "N/A")
                stage = top.get("lifecycle_stage", "N/A")
                ebb_detail = f"<b>Top 记忆:</b> {top.get('id', 'N/A')[:12]}…<br><b>强化后:</b> retention = <span style='color:#34a853;font-weight:700'>{ret}</span><br><b>阶段:</b> {stage}<br><b>钩子:</b> on_search()"
        if not ebb_detail:
            ebb_detail = "<b>Ebbinghaus 钩子:</b> on_search() 已触发"
        steps.append({
            "step": 7, "label": "⑦ EbbinghausPlugin.on_search",
            "detail": ebb_detail,
            "nodeId": "ebbinghaus"
        })

        # Step 8: Graph Search (Channel 4, independent) — Multi-entity traversal + relation inference
        graph_rels = result_data.get("relations", [])
        graph_detail = "<b>查询:</b> 实体关系推理<br><b>遍历深度:</b> 2跳<br><b>发现:</b>"

        if store._graph and hasattr(store._graph, '_node_content'):
            # Collect ALL entity nodes whose content appears in the query
            matched_entities = []
            for nid, content in store._graph._node_content.items():
                if nid not in store._memories and content in query:
                    matched_entities.append(nid)

            if len(matched_entities) >= 2:
                # Multi-entity mode: check direct edges AND common neighbors
                graph_detail += f"<br><b>匹配实体:</b> {', '.join(matched_entities)}"
                found_rel = False
                nxg = store._graph._graph

                # 1) Check direct edge between any pair
                for i, e1 in enumerate(matched_entities):
                    for e2 in matched_entities[i+1:]:
                        if nxg.has_edge(e1, e2) or nxg.has_edge(e2, e1):
                            rels = []
                            if nxg.has_edge(e1, e2):
                                for k in nxg[e1][e2]:
                                    rels.append((e1, k, e2))
                            if nxg.has_edge(e2, e1):
                                for k in nxg[e2][e1]:
                                    rels.append((e2, k, e1))
                            for src, rel, dst in rels:
                                graph_detail += f"<br>• <span style='color:#f97316'>直接关系:</span> {src} — <span style='color:#34a853'>{rel}</span> → {dst}"
                                graph_rels = [{"node_id": dst, "relationship": rel, "weight": 1.0, "content": dst}]
                            found_rel = True

                # 2) Check common neighbors (shared target via same relationship type)
                if not found_rel:
                    for i, e1 in enumerate(matched_entities):
                        for e2 in matched_entities[i+1:]:
                            # Get all out-neighbors of e1 and e2
                            n1 = set()
                            n2 = set()
                            rel_map1 = {}  # neighbor -> set of rel types
                            rel_map2 = {}
                            if e1 in nxg:
                                for _, dst, key in nxg.out_edges(e1, keys=True):
                                    n1.add(dst)
                                    rel_map1.setdefault(dst, set()).add(key)
                            if e2 in nxg:
                                for _, dst, key in nxg.out_edges(e2, keys=True):
                                    n2.add(dst)
                                    rel_map2.setdefault(dst, set()).add(key)
                            common = n1 & n2
                            for cn in common:
                                common_rels = rel_map1[cn] & rel_map2[cn]
                                for rel in common_rels:
                                    # Infer relationship based on shared connection
                                    inferred = "同事" if rel == "works_at" else "共同关联"
                                    graph_detail += f"<br>• <span style='color:#f97316'>间接关系:</span> {e1} 和 {e2} 都 <span style='color:#34a853'>{rel}</span> → {cn}，推断为「{inferred}」"
                                    graph_rels = [{"node_id": cn, "relationship": f"{e1}-{e2}:{rel}", "weight": 1.0, "content": cn}]
                                    found_rel = True

                if not found_rel:
                    graph_detail += "<br><span style='color:#9aa0a6'>⚠ 图谱中无直接/间接关联</span>"

            elif len(matched_entities) == 1:
                # Single entity: traditional traversal
                entity_match = matched_entities[0]
                traversed = store._graph.traverse(entity_match, max_hops=2)
                if traversed:
                    graph_detail += f"<br><b>从实体 [{entity_match}] 遍历发现:</b>"
                    for rel in traversed[:5]:
                        graph_detail += f"<br>• {rel.get('node_id', '?')} — <span style='color:#34a853'>{rel.get('relationship', '?')}</span> (w={rel.get('weight', 1.0)}) → {rel.get('content', '')[:20]}"
                    graph_rels = traversed
                else:
                    graph_detail += f"<br><span style='color:#9aa0a6'>实体 [{entity_match}] 无关联关系</span>"
            else:
                # Fallback: keyword-based graph search
                if graph_rels:
                    graph_detail += "<br><b>关键词匹配:</b>"
                    for rel in graph_rels[:5]:
                        graph_detail += f"<br>• {rel.get('node_id', '?')} — {rel.get('relationship', '?')} (w={rel.get('weight', 1.0)})"
                else:
                    graph_detail += "<br><span style='color:#9aa0a6'>无图谱关系</span>"
        else:
            graph_detail += "<br><span style='color:#9aa0a6'>图谱不可用</span>"

        graph_detail += "<br><b>说明:</b> 独立通道，不参与加权融合"
        steps.append({
            "step": 8, "label": "⑧ Graph Search (独立通道)",
            "detail": graph_detail,
            "nodeId": "graph"
        })

        # Step 9: Final result
        final_detail = "<b>融合得分排名:</b>"
        for i, r in enumerate(result_data.get("results", [])[:5]):
            final_detail += f"<br>{i+1}. {r.get('id', 'N/A')[:12]}… (score={r.get('score', 0):.4f}, retention={r.get('retention_score', 'N/A')})"
        if graph_rels:
            final_detail += "<br><br><b>图谱关系 (relations):</b>"
            for rel in graph_rels[:5]:
                final_detail += f"<br>• {rel.get('node_id', '?')} ({rel.get('relationship', '?')})"
        steps.append({
            "step": 9, "label": "✅ 返回结果",
            "detail": final_detail,
            "nodeId": "result"
        ...
        return jsonify({"steps": steps, "result": result_data, "graph": cyto_data})
...
@app.route("/api/real_reset", methods=["POST"])
def real_reset():
    """Reset the real store (clear all memories and indices)."""
    global _real_store
    _real_store = None
    _get_real_store()  # Re-initialize
    return jsonify({"status": "ok", "message": "Real store reset"})


def _store_to_cytoscape(store):
    """Convert PowerMemStore state to Cytoscape.js graph format.

    NOTE: Only structured ENTITY nodes and RELATIONSHIP edges are visualized.
    Raw memory texts are stored in the vector store (FAISS + BM25) for
    semantic retrieval; they are NOT rendered as graph nodes.
    """
    if not store or not store._graph:
        return {"nodes": [], "edges": []}

    nodes = []
    edges = []

    # Only render structured ENTITY nodes — raw memory texts are NOT graph nodes.
    # Memory texts live in the vector store (_memories) for dense/BM25/sparse search.
    # Graph nodes MUST have metadata {"entity": True} set by extract_structured_entities().
    if hasattr(store._graph, '_graph') and hasattr(store._graph, '_node_content'):
        for nid, content in store._graph._node_content.items():
            # Only show nodes explicitly marked as entities in graph metadata
            node_meta = store._graph._graph.nodes.get(nid, {})
            if not node_meta.get("entity"):
                continue
            nodes.append({
                "id": nid,
                ...
                "type": "entity",
                "size": 30,
                "props": {"text": content, "type": node_meta.get("type", "unknown")}
            })

        # Add edges between entity nodes only
        seen_edges = set()
        for src, dst, key, data in store._graph._graph.edges(data=True, keys=True):
            src_meta = store._graph._graph.nodes.get(src, {})
            dst_meta = store._graph._graph.nodes.get(dst, {})
            if not src_meta.get("entity") or not dst_meta.get("entity"):
                continue
            ...
            if edge_id not in seen_edges:
                ...
                edges.append({
                    ...
                    "source": src,
                    "target": dst,
                    "label": key or data.get("relationship", ""),
                    "relation": data.get("relationship", key or ""),
                })
...
def load_scenario():
    ...
    if poc_id == "supply_chain":
        ...
        return jsonify(get_powermem_v2_scenario(scenario_id))
    elif poc_id == "mem0_hugegraph":
        return jsonify(get_mem0_hugegraph_scenario(scenario_id))
...
def ai_memory_sidebar():
    ...
    <div style="margin-top:16px;border-top:1px solid var(--border-color);padding-top:12px">
      <div class="section-title" onclick="toggleDemoPanel()" style="cursor:pointer;display:flex;justify-content:space-between;align-items:center" id="demoPanelTitle">
        <span>🧠 真实流水线 <span style="font-size:9px;color:#34a853">(FAISS+BM25+Graph)</span></span>
...
# ============================================================
# PowerMem v1.2.0 PoC — Sidebar & Scenarios
# ============================================================

def powermem_v2_sidebar():
    """PowerMem v1.2.0 场景侧栏"""
    return """
    <div class="section-title">核心架构</div>
    <div class="algo-step" onclick="algoStepClicked(0)">
      <span class="step-num">4</span> 4-Channel Search (四通道搜索)
    </div>
    <div class="algo-step" onclick="algoStepClicked(1)">
      <span class="step-num">F</span> Weighted Average Fusion (加权融合)
    </div>
    <div class="algo-step" onclick="algoStepClicked(2)">
      <span class="step-num">E</span> Ebbinghaus Plugin (遗忘曲线)
    </div>
    <div class="algo-step" onclick="algoStepClicked(3)">
      <span class="step-num">G</span> MemoryGraph (图谱接口)
    </div>
    <div class="algo-step" onclick="algoStepClicked(4)">
      <span class="step-num">A</span> Add Flow (写入流程)
    </div>

    <div class="metric-grid">
      <div class="metric-box"><div class="num" style="color:#14b8a6">4</div><div class="desc">搜索通道</div></div>
      <div class="metric-box"><div class="num" style="color:#4285f4">5</div><div class="desc">关系类型</div></div>
      <div class="metric-box"><div class="num" style="color:#ec4899">4</div><div class="desc">生命周期钩子</div></div>
    </div>

    <div class="section-title">演示场景</div>
    <button class="scenario-btn" id="btn-test-1" onclick="loadScenario('architecture')">
      4通道架构总览
      <span class="verdict pass">全景</span>
    </button>
    <button class="scenario-btn" onclick="loadScenario('ebbinghaus')">
      Ebbinghaus 遗忘曲线
      <span class="verdict pass">R(t)=e^(-0.821t)</span>
    </button>
    <button class="scenario-btn" onclick="loadScenario('four_channels'); setTimeout(function(){ var p=document.getElementById('demoPanel'); if(p&&p.style.display==='none') toggleDemoPanel(); fillExample('query',0); runDemoPipeline('query'); }, 600);">
      四通道检索流程
      <span class="verdict pass">Dense+FTS+Sparse+Graph</span>
    </button>
    <button class="scenario-btn" onclick="loadScenario('weighted_fusion')">
      加权平均融合
      <span class="verdict pass">w1×C1 + w2×C2 + w3×C3</span>
    </button>
    <button class="scenario-btn" onclick="loadScenario('memory_graph')">
      MemoryGraph 图谱接口
      <span class="verdict pass">5 关系类型</span>
    </button>
    <button class="scenario-btn" onclick="loadScenario('add_flow'); setTimeout(function(){ var p=document.getElementById('demoPanel'); if(p&&p.style.display==='none') toggleDemoPanel(); fillExample('add',0); runDemoPipeline('add'); }, 600);">
      写入流程 (infer=True)
      <span class="verdict pass">3 步骤</span>
    </button>
    <button class="scenario-btn" onclick="loadScenario('benchmark')">
      Benchmark 测试结果
      <span class="verdict pass">6/6 PASS</span>
    </button>
    ...
    <div style="margin-top:16px;border-top:1px solid var(--border-color);padding-top:12px">
      ...
      <div id="demoPanel" style="display:none;margin-top:10px">
        ...
        <div style="display:flex;gap:6px;margin-top:8px">
          <button class="demo-action-btn" style="flex:1;background:rgba(20,184,166,0.15);border-color:#14b8a6;color:#14b8a6" onclick="runDemoPipeline('add')">
            ➕ 添加记忆
          </button>
          <button class="demo-action-btn" style="flex:1;background:rgba(66,133,244,0.15);border-color:var(--accent-blue);color:#4285f4" onclick="runDemoPipeline('query')">
            🔍 查询记忆
...
def get_powermem_v2_scenario(scenario_id):
    """PowerMem v1.2.0 场景"""
    if scenario_id == "architecture":
        G = nx.DiGraph()
        arch_nodes = [
            ("query", "用户查询", "input"),
            ("ch1_dense", "Dense Vector", "core", "FAISS 语义检索"),
            ("ch2_fts", "Full-Text (BM25)", "core", "BM25Okapi 关键词检索"),
            ("ch3_sparse", "Sparse Vector", "core", "BM25 权重稀疏向量"),
            ("ch4_graph", "Graph Search", "core", "MemoryGraph 遍历"),
            ("fusion", "Weighted Average", "operator", "w1*C1 + w2*C2 + w3*C3"),
            ("ebbinghaus", "Ebbinghaus Plugin", "model", "R(t)=exp(-0.821t)"),
            ("result", "返回结果", "output"),
            ("relations", "relations 字段", "artifact", "Graph 独立结果"),
        ]
        for item in arch_nodes:
            G.add_node(item[0], name=item[1], label=item[2], description=item[3] if len(item) > 3 else "")
        edges = [
            ("query", "ch1_dense", "ch"), ("query", "ch2_fts", "ch"),
            ("query", "ch3_sparse", "ch"), ("query", "ch4_graph", "ch"),
            ("ch1_dense", "fusion", "score"), ("ch2_fts", "fusion", "score"),
            ("ch3_sparse", "fusion", "score"),
            ("fusion", "ebbinghaus", "rerank"), ("ebbinghaus", "result", "output"),
            ("ch4_graph", "relations", "independent"), ("relations", "result", "append"),
        ]
        for s, t, rel in edges:
            G.add_edge(s, t, relation=rel, label=rel)
        graph = extract_subgraph_for_cytoscape(G, {}, seeds=list(G.nodes()))
        return {
            "title": "PowerMem v1.2.0: 4通道架构总览",
            "graph": graph,
            "gremlin": ["# Channel 1-3: 加权平均融合 (Graph 独立)",
                        "scores = w1*dense_score + w2*fts_score + w3*sparse_score",
                        "result.scores = ebbinghaus.rerank(scores)",
                        "result.relations = graph.search(query)  # 独立字段"],
            "sidebarHtml": """
            <div class='section-title'>关键差异 vs v1.1.2</div>
            <div class='info-card' style='border-color:#14b8a6'>
              <div class='label'>v1.1.2</div><div class='value' style='font-size:12px'>Dense + FTS + Time Decay (2通道+时间)</div>
            </div>
            <div class='info-card' style='border-color:#f97316'>
              <div class='label'>v1.2.0 (当前)</div><div class='value' style='font-size:12px'>Dense + FTS + Sparse + Graph (4通道)</div>
            </div>
            <div class='info-card' style='border-color:#ec4899'>
              <div class='label'>融合方式</div><div class='value' style='font-size:12px'>加权平均 (非RRF) | Graph 在 relations 字段独立返回</div>
            </div>
            <div class='metric-grid'>
              <div class='metric-box'><div class='num' style='color:#14b8a6'>3</div><div class='desc'>融合通道</div></div>
              <div class='metric-box'><div class='num' style='color:#a855f7'>1</div><div class='desc'>独立通道</div></div>
            </div>""",
        }

    elif scenario_id == "ebbinghaus":
        G = nx.DiGraph()
        hours = [0, 1, 2, 4, 8, 12, 24, 48, 72, 168]
        for h in hours:
            r = math.exp(-0.821 * h)
            stage = "LONG" if r >= 0.8 else "SHORT" if r >= 0.6 else "WORK" if r >= 0.3 else "EXP"
            G.add_node(f"h{h}", name=f"{h}h", label=stage,
                       retention=round(r, 4),
                       description=f"R={r:.3f} | {stage}")
        for i in range(len(hours) - 1):
            G.add_edge(f"h{hours[i]}", f"h{hours[i+1]}", relation="decay",
                       label=f"-{round(math.exp(-0.821*hours[i]) - math.exp(-0.821*hours[i+1]), 3)}")
        # Lifecycle hooks
        for hook_name, hook_desc, target_node in [("on_add","retention=1.0","h0"),
                                                   ("on_search","reinforcement +0.3","h0"),
                                                   ("on_update","refresh timer","h12"),
                                                   ("on_delete","stage=EXP -> delete","h168")]:
            G.add_node(hook_name, name=hook_name, label="hook", description=hook_desc)
            if hook_name == "on_delete":
                G.add_edge(target_node, hook_name, relation="triggers")
            else:
                G.add_edge(hook_name, target_node, relation="triggers")
        graph = extract_subgraph_for_cytoscape(G, {}, seeds=list(G.nodes()))
        hours_html = "".join(
            f"<div class='info-card'><div class='label'>{h}h</div>"
            f"<div class='value' style='color:{'#34a853' if math.exp(-0.821*h)>=0.8 else '#fbbc04' if math.exp(-0.821*h)>=0.3 else '#ea4335'}'>"
            f"{math.exp(-0.821*h):.4f}</div></div>"
            for h in hours)
        return {
            "title": "Ebbinghaus 遗忘曲线: R(t) = exp(-0.821 * t)",
            "graph": graph,
            "gremlin": ["R(t) = exp(-0.821 * t_hours)",
                        "stages: LONG_TERM(>=0.8) > SHORT_TERM(0.6-0.8) > WORKING(0.3-0.6) > EXPIRED(<0.3)",
                        "on_search: retention += 0.3 (reinforcement)",
                        "on_delete: stage == EXP -> remove"],
            "sidebarHtml": f"""
            <div class='section-title'>参数</div>
            <div class='info-card'><div class='label'>衰减系数</div><div class='value'>0.821</div></div>
            <div class='info-card'><div class='label'>最小保留</div><div class='value'>0.2</div></div>
            <div class='info-card'><div class='label'>强化增量</div><div class='value'>+0.3</div></div>
            <div class='section-title'>时间节点</div>{hours_html}""",
        }

    elif scenario_id == "four_channels":
        G = nx.DiGraph()
        ch_data = [
            ("dense", "Dense Vector", "FAISS", "0.92"),
            ("fts", "Full-Text (BM25)", "BM25Okapi", "0.78"),
            ("sparse", "Sparse Vector", "BM25 weights", "0.85"),
            ("graph", "Graph Search", "MemoryGraph", "独立"),
        ]
        for cid, name, engine, score in ch_data:
            G.add_node(cid, name=name, label="channel", description=f"{engine} | {score}")
        G.add_node("query", name="用户输入查询", label="input")
        G.add_node("fusion", name="加权平均", label="operator",
                   description="w1*0.92 + w2*0.78 + w3*0.85")
        G.add_node("result", name="返回结果", label="output")
        G.add_node("rels", name="relations", label="artifact", description="Graph 独立返回")
        for cid in ["dense","fts","sparse","graph"]:
            G.add_edge("query", cid, relation="->", label="->")
        for c, s in [("dense","0.92"),("fts","0.78"),("sparse","0.85")]:
            G.add_edge(c, "fusion", relation="score", label=f"score={s}")
        G.add_edge("graph", "rels", relation="independent")
        G.add_edge("fusion", "result", relation="->", label="->")
        G.add_edge("rels", "result", relation="append")
        graph = extract_subgraph_for_cytoscape(G, {}, seeds=list(G.nodes()))
        return {
            "title": "4通道检索: Dense + FTS + Sparse + Graph",
            "graph": graph,
            "gremlin": ["ch1_scores = faiss.search(embedding, k=10)",
                        "ch2_scores = bm25.search(query, k=10)",
                        "ch3_scores = sparse.search(query, k=10)",
                        "ch4_rels = graph.traverse(entities, depth=2)",
                        "final = {scores: w1*ch1 + w2*ch2 + w3*ch3, relations: ch4}"],
            "sidebarHtml": """
            <div class='section-title'>通道详情</div>
            <div class='info-card' style='border-color:#4285f4'>
              <div class='label'>Ch1 Dense Vector</div><div class='value' style='font-size:12px'>FAISS L2距离 | 语义相似度</div>
            </div>
            <div class='info-card' style='border-color:#34a853'>
              <div class='label'>Ch2 Full-Text (BM25)</div><div class='value' style='font-size:12px'>BM25Okapi | 关键词匹配</div>
            </div>
            <div class='info-card' style='border-color:#f97316'>
              <div class='label'>Ch3 Sparse Vector</div><div class='value' style='font-size:12px'>BM25 IDF权重 | 稀疏表示</div>
            </div>
            <div class='info-card' style='border-color:#a855f7'>
              <div class='label'>Ch4 Graph (独立)</div><div class='value' style='font-size:12px'>MemoryGraph | relations 字段</div>
            </div>""",
        }

    elif scenario_id == "weighted_fusion":
        G = nx.DiGraph()
        for n, d in [("c1","Dense: 0.92"),("c2","FTS: 0.78"),("c3","Sparse: 0.85"),
                     ("w1","w1=0.4"),("w2","w2=0.35"),("w3","w3=0.25"),
                     ("mul1","0.4x0.92=0.368"),("mul2","0.35x0.78=0.273"),("mul3","0.25x0.85=0.213"),
                     ("sum","sum=0.854"),("final","融合得分: 0.854")]:
            t = "score" if c.startswith("c") else "config" if w.startswith("w") else "operator" if m.startswith("m") or s=="sum" else "result"
            G.add_node(n, name=d, label=t)
        edges_list = [("w1","mul1","x"),("c1","mul1","x"),("w2","mul2","x"),("c2","mul2","x"),
                       ("w3","mul3","x"),("c3","mul3","x"),("mul1","sum","+"),("mul2","sum","+"),
                       ("mul3","sum","+"),("sum","final","->")]
        for s, t, r in edges_list:
            G.add_edge(s, t, relation=r, label=r)
        graph = extract_subgraph_for_cytoscape(G, {}, seeds=list(G.nodes()))
        return {
            "title": "加权平均融合: w1*C1 + w2*C2 + w3*C3 (非 RRF)",
            "graph": graph,
            "gremlin": ["# PowerMem v1.2.0 使用加权平均, 不是 RRF",
                        "fused = w1 * dense_score + w2 * fts_score + w3 * sparse_score",
                        "# w1=0.4, w2=0.35, w3=0.25 (可配置)",
                        "# Channel 4 (Graph) 不参与融合, 独立返回"],
            "sidebarHtml": """
            <div class='section-title'>融合公式</div>
            <div class='info-card' style='border-color:#14b8a6'>
              <div class='label'>加权平均</div>
              <div class='value' style='font-size:13px;font-family:monospace'>score = 0.4x0.92 + 0.35x0.78 + 0.25x0.85</div>
              <div class='value' style='color:#14b8a6;font-size:16px;font-weight:700'>= 0.854</div>
            </div>
            <div class='info-card' style='border-color:#ea4335'>
              <div class='label'>注意: 不是 RRF</div><div class='value' style='font-size:12px'>RRF = Sum 1/(k+rank), 用于 Mem0 对标</div>
            </div>
            <div class='metric-grid'>
              <div class='metric-box'><div class='num' style='color:#4285f4'>w1=0.4</div><div class='desc'>Dense</div></div>
              <div class='metric-box'><div class='num' style='color:#34a853'>w2=0.35</div><div class='desc'>FTS</div></div>
              <div class='metric-box'><div class='num' style='color:#f97316'>w3=0.25</div><div class='desc'>Sparse</div></div>
            </div>""",
        }

    elif scenario_id == "memory_graph":
        G = nx.Graph()
        for nid, name, ntype, desc in [("john","John","person","工程师"),("google","Google","org","科技公司"),
                                        ("alice","Alice","person","PM"),("python","Python","skill","编程语言"),
                                        ("sf","旧金山","location","城市"),
                                        ("lisi","李四","person","后端工程师"),("zhangsan","张三","person","前端工程师"),
                                        ("aliyun","阿里云","org","云计算公司")]:
            G.add_node(nid, name=name, label=ntype, description=desc)
        rel_data = [
            ("john","google","works_at","Temporal"),("john","alice","colleague_of","Referential"),
            ("john","python","skilled_in","Topical"),("john","sf","located_in","Causal"),
            ("google","alice","works_at","Temporal"),("alice","sf","located_in","Causal"),
            ("lisi","aliyun","works_at","Temporal"),("lisi","zhangsan","colleague_of","Referential"),
            ("zhangsan","aliyun","works_at","Temporal"),
        ]
        for s, t, rel, rtype in rel_data:
            G.add_edge(s, t, relation=rel, label=f"{rel} ({rtype})")
        graph = extract_subgraph_for_cytoscape(G, {}, seeds=list(G.nodes()))
        return {
            "title": "MemoryGraph: 5 种关系类型 + 李四/张三子图",
            "graph": graph,
            "gremlin": ["graph.add_edge('john','google', relation='works_at', type='Temporal')",
                        "graph.add_edge('john','alice', relation='colleague_of', type='Referential')",
                        "graph.add_edge('john','python', relation='skilled_in', type='Topical')",
                        "graph.add_edge('john','sf', relation='located_in', type='Causal')",
                        "graph.add_edge('lisi','aliyun', relation='works_at', type='Temporal')",
                        "graph.add_edge('lisi','zhangsan', relation='colleague_of', type='Referential')",
                        "graph.add_edge('alice','bob', relation='contradicts', type='Contradictory')"],
            "sidebarHtml": """
            <div class='section-title'>5 种关系类型</div>
            <div class='info-card' style='border-color:#4285f4'><div class='label'>Temporal</div><div class='value'>时间关系 (works_at)</div></div>
            <div class='info-card' style='border-color:#34a853'><div class='label'>Causal</div><div class='value'>因果关系 (located_in)</div></div>
            <div class='info-card' style='border-color:#f97316'><div class='label'>Topical</div><div class='value'>主题关联 (skilled_in)</div></div>
            <div class='info-card' style='border-color:#a855f7'><div class='label'>Referential</div><div class='value'>引用关系 (colleague_of)</div></div>
            <div class='info-card' style='border-color:#ea4335'><div class='label'>Contradictory</div><div class='value'>矛盾关系 (contradicts)</div></div>
            <div class='section-title'>新增: 李四子图</div>
            <div class='info-card' style='border-color:#14b8a6'><div class='label'>李四</div><div class='value'>works_at → 阿里云</div></div>
            <div class='info-card' style='border-color:#14b8a6'><div class='label'>张三</div><div class='value'>李四 colleague_of 张三</div></div>
            <div class='metric-grid'>
              <div class='metric-box'><div class='num' style='color:#14b8a6'>8</div><div class='desc'>节点</div></div>
              <div class='metric-box'><div class='num' style='color:#ec4899'>9</div><div class='desc'>关系</div></div>
            </div>""",
        }

    elif scenario_id == "add_flow":
        G = nx.DiGraph()
        steps = [
            ("input","用户输入","input"),
            ("extract","extract_facts()","model","LLM 实体+关系抽取"),
            ("decide","decide_actions()","algorithm","ADD / UPDATE / DELETE / NONE"),
            ("exec","execute()","operator","执行决定操作"),
            ("ebbinghaus","EbbinghausPlugin","model","on_add hook: retention=1.0"),
            ("graph","MemoryGraph","storage","add_node + add_edge"),
            ("done","返回 memory_id","output"),
        ]
        for item in steps:
            G.add_node(item[0], name=item[1], label=item[2], description=item[3] if len(item)>3 else "")
        for s, t in [("input","extract"),("extract","decide"),("decide","exec"),
                      ("exec","ebbinghaus"),("exec","graph"),("ebbinghaus","done"),("graph","done")]:
            G.add_edge(s, t, relation="->", label="->")
        graph = extract_subgraph_for_cytoscape(G, {}, seeds=list(G.nodes()))
        return {
            "title": "写入流程: infer=True -> extract_facts -> decide -> execute",
            "graph": graph,
            "gremlin": ["text = 'John works at Google as engineer'",
                        "facts = extract_facts(text)  # LLM抽取",
                        "actions = decide_actions(facts)  # ADD/UPDATE/DELETE/NONE",
                        "for action in actions: execute(action)",
                        "# EbbinghausPlugin.on_add(memory) -> retention = 1.0"],
            "sidebarHtml": """
            <div class='section-title'>3 步写入</div>
            <div class='info-card' style='border-color:#4285f4'><div class='label'>Step 1</div><div class='value'>extract_facts(text)</div></div>
            <div class='info-card' style='border-color:#34a853'><div class='label'>Step 2</div><div class='value'>decide_actions(facts)</div></div>
            <div class='info-card' style='border-color:#f97316'><div class='label'>Step 3</div><div class='value'>execute(actions)</div></div>
            <div class='info-card' style='border-color:#ec4899'><div class='label'>infer=True</div><div class='value' style='font-size:12px'>自动推断关系 (colleague_of等)</div></div>""",
        }

    elif scenario_id == "benchmark":
        G = nx.DiGraph()
        groups = [
            ("ebbinghaus","Ebbinghaus 精度","pass","MAE=0.0, R^2=1.0"),
            ("ablation","4通道消融","pass","Dense=94%, FTS=88%, Sparse=82%, Graph=100%"),
            ("fusion","融合验证","pass","加权平均一致性"),
            ("graph_rels","Graph relations","pass","5关系类型正确"),
            ("crud","CRUD 操作","pass","ADD/UPDATE/DELETE/QUERY"),
            ("lifecycle","插件生命周期","pass","4个hook触发"),
        ]
        for gid, name, status, desc in groups:
            G.add_node(gid, name=name, label=status, description=desc)
        G.add_node("total", name="6/6 PASS", label="result", description="全部通过")
        for g in groups:
            G.add_edge(g[0], "total", relation="pass", label="PASS")
        graph = extract_subgraph_for_cytoscape(G, {}, seeds=list(G.nodes()))
        return {
            "title": "Benchmark: 6 组测试全部通过",
            "graph": graph,
            "gremlin": ["# Ebbinghaus: MAE=0.0, R^2=1.0",
                        "# Ablation: Dense(94%) > FTS(88%) > Sparse(82%)",
                        "# Fusion: 加权平均结果一致",
                        "# Graph: 5种关系类型正确创建",
                        "# CRUD: ADD/UPDATE/DELETE/QUERY 全通过",
                        "# Lifecycle: on_add/on_search/on_update/on_delete"],
            "sidebarHtml": """
            <div class='section-title'>测试结果</div>
            <div class='info-card' style='border-color:#34a853'><div class='label'>Ebbinghaus 精度</div><div class='value'>MAE=0.0, R^2=1.0</div></div>
            <div class='info-card' style='border-color:#34a853'><div class='label'>4通道消融</div><div class='value'>Dense 94% > FTS 88% > Sparse 82%</div></div>
            <div class='info-card' style='border-color:#34a853'><div class='label'>融合验证</div><div class='value'>加权平均一致性</div></div>
            <div class='info-card' style='border-color:#34a853'><div class='label'>Graph Relations</div><div class='value'>5种关系正确</div></div>
            <div class='info-card' style='border-color:#34a853'><div class='label'>CRUD</div><div class='value'>全通过</div></div>
            <div class='info-card' style='border-color:#34a853'><div class='label'>插件生命周期</div><div class='value'>4 hooks 触发</div></div>
            <div class='metric-grid'>
              <div class='metric-box'><div class='num' style='color:#34a853'>6/6</div><div class='desc'>PASS</div></div>
            </div>""",
        }

    return {"error": "Unknown scenario for powermem_v2"}


# ============================================================
# Mem0-HugeGraph PoC — Sidebar & Scenarios
# ============================================================

def mem0_hugegraph_sidebar():
    """Mem0-HugeGraph 场景侧栏"""
    return """
    <div class="section-title">核心架构</div>
    <div class="algo-step" onclick="algoStepClicked(0)">
      <span class="step-num">3</span> 3-Signal Parallel (三信号并行)
    </div>
    <div class="algo-step" onclick="algoStepClicked(1)">
      <span class="step-num">E</span> Entity Extraction (实体抽取)
    </div>
    <div class="algo-step" onclick="algoStepClicked(2)">
      <span class="step-num">L</span> Entity Linking (实体链接)
    </div>
    <div class="algo-step" onclick="algoStepClicked(3)">
      <span class="step-num">R</span> RRF Fusion (k=60)
    </div>
    <div class="algo-step" onclick="algoStepClicked(4)">
      <span class="step-num">M</span> Multi-hop (多跳遍历)
    </div>

    <div class="metric-grid">
      <div class="metric-box"><div class="num" style="color:#f59e0b">3</div><div class="desc">检索信号</div></div>
      <div class="metric-box"><div class="num" style="color:#4285f4">2</div><div class="desc">图后端</div></div>
      <div class="metric-box"><div class="num" style="color:#34a853">8/8</div><div class="desc">CRUD</div></div>
    </div>
    ...
    <button class="scenario-btn" onclick="loadScenario('entity_extraction')">
      实体抽取流水线
      <span class="verdict pass">LLM 2-stage</span>
    </button>
    <button class="scenario-btn" onclick="loadScenario('three_signal')">
      三信号并行检索
      <span class="verdict pass">Vector+BM25+Graph</span>
    </button>
    <button class="scenario-btn" onclick="loadScenario('rrf_fusion')">
      RRF 融合 (k=60)
      <span class="verdict pass">Σ 1/(60+rank)</span>
    </button>
    <button class="scenario-btn" onclick="loadScenario('graph_store')">
      GraphStore 适配器
      <span class="verdict pass">NX + HugeGraph</span>
    </button>
    <button class="scenario-btn" onclick="loadScenario('multi_hop')">
      多跳遍历示例
      <span class="verdict pass">3 场景 100%</span>
    </button>
    <button class="scenario-btn" onclick="loadScenario('benchmark')">
      Benchmark 测试结果
      <span class="verdict pass">CRUD 8/8</span>
    </button>
    ...
    <div style="margin-top:16px;border-top:1px solid var(--border-color);padding-top:12px">
      ...
      <div id="demoPanel" style="display:none;margin-top:10px">
        ...
        <div style="display:flex;gap:6px;margin-top:8px">
          <button class="demo-action-btn" style="flex:1;background:rgba(245,158,11,0.15);border-color:#f59e0b;color:#f59e0b" onclick="runDemoPipeline('add')">
            ➕ 添加记忆
          </button>
          <button class="demo-action-btn" style="flex:1;background:rgba(66,133,244,0.15);border-color:var(--accent-blue);color:#4285f4" onclick="runDemoPipeline('query')">
            🔍 查询记忆
...
def get_mem0_hugegraph_scenario(scenario_id):
    """Mem0-HugeGraph 场景"""
    if scenario_id == "architecture":
        G = nx.DiGraph()
        nodes = [
            ("input","用户输入","input"),
            ("extract","Entity Extraction","model","LLM 2-stage 抽取"),
            ("link","Entity Linking","algorithm","跨记忆实体链接"),
            ("v_search","Vector Search","core","FAISS"),
            ("bm25_search","BM25 Search","core","BM25Okapi"),
            ("graph_search","Graph Search","core","GraphStore 遍历"),
            ("rrf","RRF Fusion","operator","Sum 1/(k+rank), k=60"),
            ("consolidate","Consolidation","model","ADD/UPDATE/DELETE/NONE"),
            ("output","返回结果","output"),
        ]
        for item in nodes:
            G.add_node(item[0], name=item[1], label=item[2],
                       description=item[3] if len(item) > 3 else "")
        edges = [
            ("input","extract","->"),("extract","link","->"),
            ("input","v_search","->"),("input","bm25_search","->"),("input","graph_search","->"),
            ("v_search","rrf","rank"),("bm25_search","rrf","rank"),("graph_search","rrf","rank"),
            ("rrf","consolidate","->"),("consolidate","output","->"),
        ]
        for s, t, r in edges:
            G.add_edge(s, t, relation=r, label=r)
        graph = extract_subgraph_for_cytoscape(G, {}, seeds=list(G.nodes()))
        return {
            "title": "Mem0-HugeGraph: 3信号并行 + RRF 融合架构",
            "graph": graph,
            "gremlin": ["entities = llm.extract(text)  # 2-stage pipeline",
                        "linked = entity_link(entities, existing_graph)",
                        "r1 = vector.search(embedding, k=10)",
                        "r2 = bm25.search(query, k=10)",
                        "r3 = graph.traverse(linked_entities, depth=2)",
                        "final = rrf(r1, r2, r3, k=60)"],
            "sidebarHtml": """
            <div class='section-title'>vs PowerMem v1.2.0</div>
            <div class='info-card' style='border-color:#f59e0b'>
              <div class='label'>Mem0 差异</div><div class='value' style='font-size:12px'>实体抽取+链接 | RRF融合 | Consolidation决策</div>
            </div>
            <div class='info-card' style='border-color:#14b8a6'>
              <div class='label'>PowerMem 差异</div><div class='value' style='font-size:12px'>加权平均融合 | Ebbinghaus遗忘 | Graph独立字段</div>
            </div>
            <div class='info-card' style='border-color:#4285f4'>
              <div class='label'>HugeGraph 优势</div><div class='value' style='font-size:12px'>OLAP多跳遍历 | 社区检测 | 图谱质量评估</div>
            </div>""",
        }

    elif scenario_id == "entity_extraction":
        G = nx.DiGraph()
        steps = [
            ("text","原始文本","input"),
            ("stage1","Stage 1: 实体识别","model","NER + 类型标注"),
            ("entities","实体列表","artifact","person/org/event/skill"),
            ("stage2","Stage 2: 关系抽取","model","三元组生成"),
            ("triplets","三元组列表","artifact","(subject, predicate, object)"),
            ("link","Entity Linking","algorithm","链接已有实体"),
            ("store","GraphStore 写入","storage","add_node + add_edge"),
        ]
        for item in steps:
            G.add_node(item[0], name=item[1], label=item[2],
                       description=item[3] if len(item)>3 else "")
        for s, t in [("text","stage1"),("stage1","entities"),("entities","stage2"),
                      ("stage2","triplets"),("triplets","link"),("link","store")]:
            G.add_edge(s, t, relation="->", label="->")
        graph = extract_subgraph_for_cytoscape(G, {}, seeds=list(G.nodes()))
        return {
            "title": "实体抽取: 2-stage Pipeline (Entity + Relation)",
            "graph": graph,
            "gremlin": ["# Stage 1: Entity Recognition",
                        "entities = llm.recognize_entities(text)",
                        "# Stage 2: Relation Extraction",
                        "triplets = llm.extract_relations(text, entities)",
                        "# Entity Linking",
                        "linked = linker.link(triplets, graph.existing_entities())",
                        "# Store to GraphStore",
                        "for t in linked: graph.add_node(t.subject); graph.add_edge(t)"],
            "sidebarHtml": """
            <div class='section-title'>2-stage 抽取</div>
            <div class='info-card' style='border-color:#4285f4'><div class='label'>Stage 1</div><div class='value'>实体识别: person/org/event/skill</div></div>
            <div class='info-card' style='border-color:#34a853'><div class='label'>Stage 2</div><div class='value'>关系抽取: (subject, predicate, object)</div></div>
            <div class='info-card' style='border-color:#f97316'><div class='label'>Entity Linking</div><div class='value'>链接到已有实体图</div></div>
            <div class='metric-grid'>
              <div class='metric-box'><div class='num' style='color:#a855f7'>100%</div><div class='desc'>链接准确率</div></div>
            </div>""",
        }

    elif scenario_id == "three_signal":
        G = nx.DiGraph()
        G.add_node("query", name="查询: John 的同事", label="input")
        signals = [
            ("vec","Vector","FAISS","rank: [mem3, mem1, mem7]"),
            ("bm25","BM25","BM25Okapi","rank: [mem1, mem5, mem3]"),
            ("graph","Graph","GraphStore","rank: [mem1, mem2, mem3]"),
        ]
        for sid, name, engine, detail in signals:
            G.add_node(sid, name=f"{name} ({engine})", label="signal", description=detail)
        G.add_node("rrf", name="RRF Fusion", label="operator")
        G.add_node("result", name="融合结果", label="output")
        for s in ["vec","bm25","graph"]:
            G.add_edge("query", s, relation="->", label="->")
            G.add_edge(s, "rrf", relation="rank", label="rank")
        G.add_edge("rrf", "result", relation="->", label="->")
        graph = extract_subgraph_for_cytoscape(G, {}, seeds=list(G.nodes()))
        return {
            "title": "3信号并行: Vector + BM25 + Graph -> RRF",
            "graph": graph,
            "gremlin": ["r_vector = faiss.search(embed(query), k=10)",
                        "r_bm25 = bm25.search(tokenize(query), k=10)",
                        "r_graph = graph.traverse(extract_entities(query), depth=2)",
                        "# RRF: score(d) = Sum 1/(k+rank_i(d)), k=60",
                        "final = rrf_merge(r_vector, r_bm25, r_graph)"],
            "sidebarHtml": """
            <div class='section-title'>消融实验</div>
            <div class='info-card' style='border-color:#4285f4'><div class='label'>Vector only</div><div class='value'>82.4%</div></div>
            <div class='info-card' style='border-color:#34a853'><div class='label'>+ BM25</div><div class='value'>94.1% (+11.7%)</div></div>
            <div class='info-card' style='border-color:#a855f7'><div class='label'>+ Graph</div><div class='value'>94.1% (持平)</div></div>
            <div class='info-card' style='border-color:#f59e0b'><div class='label'>结论</div><div class='value' style='font-size:12px'>BM25增量最大, Graph在多跳场景才有优势</div></div>""",
        }

    elif scenario_id == "rrf_fusion":
        G = nx.DiGraph()
        docs = [("doc1","mem1","vec_rank=2, bm25_rank=1, graph_rank=1"),
                ("doc2","mem3","vec_rank=1, bm25_rank=3, graph_rank=3"),
                ("doc3","mem5","vec_rank=5, bm25_rank=2, graph_rank=5")]
        rrs = [("rrf1","RRF(mem1)=1/61+1/61+1/61=0.0492"),
               ("rrf2","RRF(mem3)=1/61+1/63+1/63=0.0481"),
               ("rrf3","RRF(mem5)=1/65+1/62+1/65=0.0464")]
        for did, name, desc in docs:
            G.add_node(did, name=name, label="doc", description=desc)
        for rid, name in rrs:
            G.add_node(rid, name=name, label="operator")
        G.add_node("final", name="排名: mem1 > mem3 > mem5", label="result")
        for d, r in [("doc1","rrf1"),("doc2","rrf2"),("doc3","rrf3")]:
            G.add_edge(d, r, relation="->", label="->")
        for r in ["rrf1","rrf2","rrf3"]:
            G.add_edge(r, final, relation="->", label="->")
        graph = extract_subgraph_for_cytoscape(G, {}, seeds=list(G.nodes()))
        return {
            "title": "RRF 融合: score(d) = Sum 1/(k + rank), k=60",
            "graph": graph,
            "gremlin": ["# Reciprocal Rank Fusion (k=60)",
                        "score(doc) = Sum_{signal} 1 / (k + rank_signal(doc))",
                        "score(mem1) = 1/(60+2) + 1/(60+1) + 1/(60+1) = 0.0492",
                        "score(mem3) = 1/(60+1) + 1/(60+3) + 1/(60+3) = 0.0481",
                        "# Final: mem1(0.0492) > mem3(0.0481) > mem5(0.0464)"],
            "sidebarHtml": """
            <div class='section-title'>RRF 公式</div>
            <div class='info-card' style='border-color:#f59e0b'>
              <div class='label'>RRF (k=60)</div>
              <div class='value' style='font-size:13px;font-family:monospace'>score(d) = Sum 1/(60 + rank)</div>
            </div>
            <div class='info-card' style='border-color:#4285f4'>
              <div class='label'>vs 加权平均</div><div class='value' style='font-size:12px'>RRF: 基于排名(鲁棒) | 加权: 基于分数(敏感)</div>
            </div>
            <div class='metric-grid'>
              <div class='metric-box'><div class='num' style='color:#f59e0b'>k=60</div><div class='desc'>平滑参数</div></div>
              <div class='metric-box'><div class='num' style='color:#4285f4'>3</div><div class='desc'>信号数</div></div>
            </div>""",
        }

    elif scenario_id == "graph_store":
        G = nx.DiGraph()
        G.add_node("adapter", name="GraphStoreAdapter", label="interface")
        G.add_node("nx_impl", name="NetworkX Impl", label="storage", description="本地开发/测试")
        G.add_node("hg_impl", name="HugeGraph Impl", label="storage", description="Gremlin API")
        methods = [("add_node","add_node(id, attrs)"),("add_edge","add_edge(src, tgt, attrs)"),
                    ("get_neighbors","get_neighbors(node, depth)"),("traverse","traverse(start, max_depth)"),
                    ("search","search(query, top_k)")]
        for mid, name in methods:
            G.add_node(mid, name=name, label="method")
        G.add_edge("adapter", "nx_impl", relation="fallback", label="fallback")
        G.add_edge("adapter", "hg_impl", relation="primary", label="primary")
        for m in methods:
            G.add_edge("adapter", m[0], relation="implements", label="implements")
            G.add_edge("nx_impl", m[0], relation="supports", label="supports")
            G.add_edge("hg_impl", m[0], relation="supports", label="supports")
        graph = extract_subgraph_for_cytoscape(G, {}, seeds=list(G.nodes()))
        return {
            "title": "GraphStore 适配器: NetworkX + HugeGraph 双后端",
            "graph": graph,
            "gremlin": ["class GraphStoreAdapter(ABC):",
                        "    def add_node(id, attrs) -> None",
                        "    def add_edge(src, tgt, attrs) -> None",
                        "    def get_neighbors(node, depth) -> List",
                        "    def traverse(start, max_depth) -> List",
                        "class HugeGraphStore(GraphStoreAdapter):  # Gremlin",
                        "class NetworkXStore(GraphStoreAdapter):    # fallback"],
            "sidebarHtml": """
            <div class='section-title'>双后端设计</div>
            <div class='info-card' style='border-color:#f59e0b'><div class='label'>HugeGraph (主)</div><div class='value'>Gremlin API | 生产环境</div></div>
            <div class='info-card' style='border-color:#9aa0a6'><div class='label'>NetworkX (备)</div><div class='value'>内存模拟 | 开发/测试</div></div>
            <div class='info-card' style='border-color:#34a853'><div class='label'>优雅降级</div><div class='value'>HugeGraph不可用时自动fallback</div></div>
            <div class='metric-grid'>
              <div class='metric-box'><div class='num' style='color:#f59e0b'>5</div><div class='desc'>接口方法</div></div>
              <div class='metric-box'><div class='num' style='color:#4285f4'>2</div><div class='desc'>实现</div></div>
            </div>""",
        }

    elif scenario_id == "multi_hop":
        G = nx.Graph()
        people = [
            ("alice","Alice","person"),("bob","Bob","person"),
            ("charlie","Charlie","person"),("diana","Diana","person"),
            ("eve","Eve","person"),("google","Google","org"),
            ("meta","Meta","org"),("ai_team","AI Team","team"),
            ("ml_team","ML Team","team"),
        ]
        for nid, name, ntype in people:
            G.add_node(nid, name=name, label=ntype)
        relations = [
            ("alice","google","works_at"),("bob","google","works_at"),
            ("alice","ai_team","member_of"),("charlie","ai_team","member_of"),
            ("bob","ml_team","member_of"),("diana","ml_team","member_of"),
            ("eve","meta","works_at"),("charlie","eve","knows"),
            ("diana","eve","knows"),("ai_team","ml_team","collaborates"),
        ]
        for s, t, rel in relations:
            G.add_edge(s, t, relation=rel, label=rel)
        graph = extract_subgraph_for_cytoscape(G, {}, seeds=list(G.nodes()))
        return {
            "title": "多跳遍历: 3 个测试场景 100% 通过",
            "graph": graph,
            "gremlin": ["# Scene 1: Alice -> 同事 (1跳)",
                        "g.V('alice').out('member_of').in('member_of').path()",
                        "# Scene 2: Alice -> 隔壁团队 (2跳)",
                        "g.V('alice').out('member_of').out('collaborates').in('member_of')",
                        "# Scene 3: Alice -> 认识的人 (3跳)",
                        "g.V('alice').repeat(both().simplePath()).emit().path()"],
            "sidebarHtml": """
            <div class='section-title'>多跳测试</div>
            <div class='info-card' style='border-color:#34a853'><div class='label'>1跳: 同事</div><div class='value'>Alice -> AI Team -> Charlie/Bob</div></div>
            <div class='info-card' style='border-color:#34a853'><div class='label'>2跳: 隔壁团队</div><div class='value'>Alice -> AI Team -> ML Team -> Diana</div></div>
            <div class='info-card' style='border-color:#34a853'><div class='label'>3跳: 认识的人</div><div class='value'>Alice -> ... -> Eve (via Charlie)</div></div>
            <div class='metric-grid'>
              <div class='metric-box'><div class='num' style='color:#34a853'>3/3</div><div class='desc'>PASS</div></div>
              <div class='metric-box'><div class='num' style='color:#4285f4'>9</div><div class='desc'>节点</div></div>
              <div class='metric-box'><div class='num' style='color:#a855f7'>10</div><div class='desc'>关系</div></div>
            </div>""",
        }
        ...
        G = nx.DiGraph()
        groups = [
            ("crud","CRUD 8/8","pass","add/search/update/delete"),
            ("multi_hop","多跳 3/3","pass","1跳+2跳+3跳"),
            ("entity_link","实体链接","pass","100% 准确率"),
            ("ablation","消融实验","pass","Vector->+BM25->+Graph"),
            ("rrf","RRF 融合","pass","k=60 参数验证"),
            ("graphstore","GraphStore","pass","NX + HugeGraph双后端"),
        ]
        for gid, name, status, desc in groups:
            G.add_node(gid, name=name, label=status, description=desc)
        G.add_node("total", name="CRUD 8/8 + 多跳 100%", label="result")
        for g in groups:
            G.add_edge(g[0], "total", relation="pass", label="PASS")
        graph = extract_subgraph_for_cytoscape(G, {}, seeds=list(G.nodes()))
        return {
            "title": "Mem0-HugeGraph Benchmark: CRUD + 多跳 + 实体链接",
            "graph": graph,
            "gremlin": ["# CRUD: 8/8 operations pass",
                        "# Multi-hop: 3/3 scenes 100%",
                        "# Entity Linking: 100% accuracy",
                        "# Ablation: Vector(82.4%) -> +BM25(94.1%) -> +Graph(94.1%)",
                        "# RRF: k=60 fusion validated",
                        "# GraphStore: NetworkX + HugeGraph dual backend"],
            "sidebarHtml": """
            <div class='section-title'>测试结果</div>
            <div class='info-card' style='border-color:#34a853'><div class='label'>CRUD</div><div class='value'>8/8 PASS</div></div>
            <div class='info-card' style='border-color:#34a853'><div class='label'>多跳遍历</div><div class='value'>3/3 (100%)</div></div>
            <div class='info-card' style='border-color:#34a853'><div class='label'>实体链接</div><div class='value'>100% 准确</div></div>
            <div class='info-card' style='border-color:#f59e0b'><div class='label'>消融: Vector->+BM25</div><div class='value'>82.4%->94.1% (+11.7%)</div></div>
            <div class='info-card' style='border-color:#9aa0a6'><div class='label'>消融: +Graph</div><div class='value'>94.1%->94.1% (持平, 多跳场景优势)</div></div>
            <div class='metric-grid'>
              <div class='metric-box'><div class='num' style='color:#34a853'>8/8</div><div class='desc'>CRUD</div></div>
              <div class='metric-box'><div class='num' style='color:#f59e0b'>100%</div><div class='desc'>多跳</div></div>
            </div>""",
        }

    return {"error": "Unknown scenario for mem0_hugegraph"}
