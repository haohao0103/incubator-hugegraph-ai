...
## Semantic Enrichment (fill only if the schema declares the slot)
Some schemas declare extra properties that carry LLM-generated semantics rather
than literal facts copied from the text. Fill them ONLY when the schema declares
them in the corresponding properties list:
- `description` (vertex property): 1-3 sentences summarising what the entity is
  and why it matters in this text. Ground it in the text; never invent facts.
- `strength` (edge property): an integer from 1 to 10 scoring how strongly the
  source and target entities are related (10 = core/inseparable, 1 = incidental
  mention). Use it to rank relationships, not to restate the label.
These values are generated by you, so they need not appear verbatim in the text.
If the schema does not declare them, omit them entirely.
...
## 语义增强（仅当 schema 声明了该属性时填写）
部分 schema 会声明额外属性，用于承载由你生成的语义信息，而非从原文照抄的事实。
只有当 schema 的对应 properties 列表中声明了这些属性时才填写：
- `description`（顶点属性）：用 1-3 句话概括该实体是什么、在本文本中为何重要。
  必须基于文本内容，不得编造事实。
- `strength`（边属性）：1 到 10 的整数，表示源实体与目标实体的关联强度
  （10 = 核心/密不可分，1 = 顺带提及）。用于关系排序，不要用来复述边标签。
这些值由你生成，因此无需在原文中逐字出现。
如果 schema 未声明这些属性，则完全不要输出它们。
